...
- **Syndication**: `api/news-feed.mjs` (Vercel function) serves the live published feed as RSS 2.0 at `/news/feed.xml` (rewrite in `vercel.json`) or JSON via `/api/news-feed?format=json`. Anon-key `get_news_feed` RPC, 15-min CDN cache (`s-maxage=900`), CORS `*`. Supports `?limit=` (max 200), `?category=`, `?platform=`. Item links point at `/news?story={slug}`.
